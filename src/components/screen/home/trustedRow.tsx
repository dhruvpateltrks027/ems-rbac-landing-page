import { HoverCard } from "@/components/common/animation";
      ...
      <div className="grid items-center gap-3 rounded-xl border border-border/70 shadow-sm bg-white/95 px-5 py-4 sm:grid-cols-[1.2fr_repeat(4,minmax(0,1fr))]">
        ...
        {trustedBrands.map((brand) => (
          <HoverCard key={brand.name}>
            <div
              key={brand.name}
              className="flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700"
            >
              <span
